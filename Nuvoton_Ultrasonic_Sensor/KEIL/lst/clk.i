...
uint32_t CLK_EnablePLL(uint32_t u32PllClkSrc, uint32_t u32PllFreq)
{
    ...
    if (u32PllClkSrc == (0x0ul << (19)))
    {
        ...
        u32ClkSrc = (0x0ul << (19));
        u32PllSrcClk = (12000000UL);//12 MHz
    }
    ...
    else if (u32PllClkSrc == (0x1ul << (19)))
    {
        ...
        u32ClkSrc = (0x1ul << (19));
        u32PllSrcClk = ((48000000UL) >> 2); //12 MHz
    }
    else
    {
        ...
        for (u32NO = 0UL; u32NO <= 3UL; u32NO++)
        {
            ...
            if (u32NO != 2UL)
            {
                ...
                for (u32NR = 1UL; u32NR <= 6UL; u32NR++) //u32PllSrcClk: MIRC:4M, HIRC_DIV4:12M, HXT:4~24M, 4 MHz <= u32Tmp
                {
                    ...
                    if (u32Tmp >= 4000000UL && u32Tmp <= 8000000UL) //4 MHz <= u32Tmp <= 8 MHz
                    {
                        for (u32NF = 8UL; u32NF <= 25UL; u32NF++) //4 MHz <= u32Tmp, u32Tmp2 <= 100000000UL
                        {
